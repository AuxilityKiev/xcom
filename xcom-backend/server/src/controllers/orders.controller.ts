...
export class OrdersController {
    ...
    @Inject()
    private readonly goods!: GoodRepository
    ...
    @UseBefore(
        ProxyMiddleware(ECOM_URL, {
            ...
            proxyReqPathResolver: async (ctx: Context) => {
                ...
                const ecomParams = {
                    fields: 'id,date,storeId,statusId,payType,extId',
    ...
    public async changeOrderStatus(
        ...
        const order: Order = await this.ecom.getOrderById(orderId)
        if (!order) {
        ...
        if (!comparePhoneNumbers(order.clientTel, user.MobilePhone!, 'RU')) {
    ...
    public async postFiscalCheque(
        ...
        const cheque: ManzanaCheque = await this.manzanaPosService.getCheque(request)
        const storeInn: INN | null = await this.stores.getInn(request.storeId)
        if (!storeInn) {
            throw new NotFoundError(this.localizationManager.getValue(6, request.storeId))
        }
        // store order in local db and assign local id
        const order: Order = await this.ordersRepository.insert(
            await createEcomOrder(request, cheque, payType, storeInn.INN)
        )
        switch (order.payType) {
            case PayType.CASH:
            ...
            case PayType.ONLINE:
                const authResponse = await this.sbolService.registerPreAuth({
                    ...
                    amount: cheque.amountDiscounted * 100,
