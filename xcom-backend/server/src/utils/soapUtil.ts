...
import { ManzanaChequeItem } from '../manzana/manzanaChequeItem'
...
export default class SoapUtil {
    ...
    public async sendRequest(url: string, chequeRequest: SoftChequeRequest): Promise<ManzanaCheque> {
        ...
        const xmlResponse: string = await this.sendSoapRequest(url, xmlData, {
            'Content-Type': 'text/xml;charset=UTF-8'
        })
        const response: ChequeResponseModel = converter.xml2js(xmlResponse, {
            compact: true,
            alwaysChildren: true
        }) as ChequeResponseModel
        ...
        return {
            ...
            responseRaw: xmlResponse,
            basket: items.map(
                (item, index): ManzanaChequeItem => {
                    return {
                        ...
                        amount: parseFloat(item.Summ._text),
                        amountDiscounted: parseFloat(item.SummDiscounted._text),
    ...
    private async addItems(chequeRequest: SoftChequeRequest, data: any): Promise<void> {
        ...
        for (const [index, item] of chequeRequest.basket.entries()) {
            const stock: Stock | null | undefined = await this.stocksRepository.collection.findOne({
            ...
            if (!stock) {
                ...
                prices.push({ goodsId: stock.goodsId, price: stock.ecomPrice, batchId: stock.batch })
            }
        }
        ...
        let summ: number = 0
        for (const [index, item] of chequeRequest.basket.entries()) {
            const priceDescriptor: PriceDescriptor | undefined = prices.find(it => {
                if (it.batchId && item.batchId) {
                    return it.goodsId! === item.goodsId! && it.batchId === item.batchId
                }
    ...
    private async sendSoapRequest(url: string, xml: string, headers?: SoapHeaders): Promise<any> {
        logDebug(`manzana request`, { context: __filename, body: xml })
            ...
            ntlmRequest.post(
                ...
                (err: Error, response: any) => {
                    if (err) {
                        ...
                        logDebug(`manzana response`, { context: __filename, body: response.body })
