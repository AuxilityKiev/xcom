    ...
    <v-dialog v-model="visible" max-width="500px">
      <v-card>
        ...
        <v-alert
          ...
          :type="!fileUploadedErrorMessage ? 'success' : 'error'"
        >{{ !fileUploadedErrorMessage ? $vuetify.t('$vuetify.fileSuccess') : fileUploadedErrorMessage }}</v-alert>
      </v-card>
    </v-dialog>
  </v-btn>
</template>
    ...
    methods: {
        ...
        uploadFile() {
            ...
            this.$store.dispatch(this.action, formData).then(res => {
                ...
                this.fileUploaded = true
                this.fileUploadedErrorMessage = err.message
            })
        },
        close() {
            this.file = null
